...
// Every code this app prints is one of two things: a product (numeric
// legacyCode, e.g. "1023") or a lot (always "LOT-..." — covers both
// GRN-received and production lots, which getLotDetail already
// disambiguates internally). A phone-camera QR scan yields a full
// "{origin}/products/{code}" or "{origin}/lots/{lotNo}" URL; a barcode-gun
// scan yields the bare code with no URL. Check the URL shape first, then
// fall back to the bare-code prefix/format heuristic.
export function classifyScan(scanned: string): { type: "product" | "lot"; code: string } {
